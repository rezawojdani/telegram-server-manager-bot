...
clear
echo "=============================================="
echo "      TELEGRAM BOT SERVER MANAGER 🚀         "
echo "=============================================="
echo ""
echo "Please select an option:"
echo "  1) 🚀 Install Telegram Bot Server Manager"
echo "  2) 🗑️  Remove Telegram Server Manager"
echo ""
...
if [ "$CHOICE" = "2" ]; then
    echo ""
    echo "⚠️  WARNING: This will completely remove the bot service and all files."
    ...
    sudo systemctl stop myserverbot.service 2>/dev/null
    sudo systemctl disable myserverbot.service 2>/dev/null
    sudo rm -f /etc/systemd/system/myserverbot.service
    sudo systemctl daemon-reload
    echo "🗑️  Removing bot files..."
    sudo rm -rf /opt/myserverbot
    echo ""
    echo "✅ Telegram Bot Server Manager removed successfully."
    exit 0
fi
...
sudo apt update && sudo apt install -y python3 python3-pip python3-venv ufw curl wget jq speedtest-cli

echo "📂 Creating bot home directory..."
sudo systemctl stop myserverbot.service 2>/dev/null
sudo rm -rf /opt/myserverbot
sudo mkdir -p /opt/myserverbot
sudo chown -R $USER:$USER /opt/myserverbot
cd /opt/myserverbot
...
pip install pyTelegramBotAPI psutil requests qrcode[pil] Pillow
...
cat << 'EOFCONFIG' > config.json
{
  "cpu_alert_threshold": 80,
  "cpu_alert_enabled": true,
  "cpu_alert_cooldown_minutes": 10
}
EOFCONFIG

echo "🛡️ Installing SSH Login Guard Alert..."
sudo mkdir -p /etc/ssh
sudo bash -c 'cat << '"'"'EOF_SSH'"'"' > /etc/ssh/sshrc
#!/bin/bash
if [ -f /opt/myserverbot/.env ]; then
    BOT_TOKEN=$(grep BOT_TOKEN /opt/myserverbot/.env | cut -d'"'"'"'"'"'"'"'"' -f2)
    ADMIN_ID=$(grep ADMIN_ID /opt/myserverbot/.env | cut -d'"'"'"'"'"'"'"'"' -f2)
    IP=$(echo $SSH_CONNECTION | awk '"'"'{print $1}'"'"')
    USER_LOGIN=$(whoami)
    HOSTNAME_VAL=$(hostname)
    NOW=$(date '"'"'+%Y-%m-%d %H:%M:%S'"'"')
    TEXT="🚨 *SSH LOGIN ALERT* 🚨%0A%0A🖥️ *Server:* $HOSTNAME_VAL%0A👤 *User:* $USER_LOGIN%0A🌐 *IP Address:* $IP%0A📅 *Timestamp:* $NOW"
    curl -s -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" -d "chat_id=${ADMIN_ID}" -d "text=${TEXT}" -d "parse_mode=Markdown" > /dev/null 2>&1
fi
EOF_SSH'
...
sudo bash -c "cat << EOF_SERVICE > /etc/systemd/system/myserverbot.service
[Unit]
...
Type=simple
User=$USER
WorkingDirectory=/opt/myserverbot
ExecStart=/opt/myserverbot/venv/bin/python3 /opt/myserverbot/bot.py
Restart=always
RestartSec=10
...
echo "=============================================="
echo "📌 Service: myserverbot.service"
echo "📌 Logs:    sudo journalctl -u myserverbot -f"
echo "=============================================="
